...
class MainActivity : Activity() {
    companion object { private const val REQUEST_CORE=100; private const val REQUEST_LOCATION=101; private const val CREATE_CSV=200; private const val OPEN_FIRMWARE=300 }
    ...
    private val csvExporter=CsvExporter(); private var service:RideRecordingService?=null; private var bound=false; private var rendering=false
    ...
    private fun exportSession(s:RideSession){pendingExportSessionId=s.id;val d=SimpleDateFormat("yyyyMMdd-HHmmss",Locale.US).format(Date(s.startTime));startActivityForResult(Intent(Intent.ACTION_CREATE_DOCUMENT).apply{type="text/csv";putExtra(Intent.EXTRA_TITLE,"ducati-telemetry-$d.csv")},CREATE_CSV)}
    ...
    @Deprecated("Activity Result API can be introduced later.") override fun onActivityResult(code:Int,result:Int,data:Intent?){super.onActivityResult(code,result,data);if(result!=RESULT_OK)return;val uri=data?.data?:return;when(code){CREATE_CSV->try{val source=service?.sessionFile(pendingExportSessionId?:return)?:error("Saved ride is unavailable");val out=contentResolver.openOutputStream(uri)?:error("Cannot create export file");csvExporter.export(source,out);Toast.makeText(this,"Ride exported",Toast.LENGTH_SHORT).show()}catch(e:Exception){showError(e)}finally{pendingExportSessionId=null};OPEN_FIRMWARE->startFirmwareUpdate(uri)}}
